...
class TirController extends ChangeNotifier {
  ...
  void calcularTIRM(
      List<double> flujos, double tasaReinversion, double tasaFinanciamiento) {
    try {
      _error = '';
      if (flujos.isEmpty) throw Exception('Ingrese al menos un flujo de caja');

      _resultado =
          _service.calcularTIRM(flujos, tasaReinversion, tasaFinanciamiento) *
              100;
      notifyListeners();
    } catch (e) {
  ...
  void calcularTRA(double beneficioNetoPromedio, double inversionInicial) {
    try {
      ...
      if (inversionInicial <= 0)
        throw Exception('La inversión inicial debe ser mayor a cero');

      _resultado =
          _service.calcularTRA(beneficioNetoPromedio, inversionInicial) * 100;
      notifyListeners();
    } catch (e) {
  ...
  void calcularTRR(double tasaLibreRiesgo, double primaRiesgo, double beta) {
    try {
      _error = '';
      _resultado =
          _service.calcularTRR(tasaLibreRiesgo, primaRiesgo, beta) * 100;
      notifyListeners();
    } catch (e) {
  ...
  void calcularRetornoReal(double retornoNominal, double inflacion) {
    try {
      _error = '';
      _resultado =
          _service.calcularRetornoReal(retornoNominal, inflacion) * 100;
      notifyListeners();
    } catch (e) {
  ...
  void calcularRetornoEsperado(
      List<double> posiblesRetornos, List<double> probabilidades) {
    try {
      _error = '';
      if (posiblesRetornos.isEmpty)
        throw Exception('Ingrese al menos un posible retorno');
      if (posiblesRetornos.length != probabilidades.length) {
        throw Exception(
            'Retornos y probabilidades deben tener la misma cantidad');
      }

      _resultado =
          _service.calcularRetornoEsperado(posiblesRetornos, probabilidades) *
              100;
      notifyListeners();
    } catch (e) {
